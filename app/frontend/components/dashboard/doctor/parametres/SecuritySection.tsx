...
export function TelemedicineSection() {
  ...
  const [fees, setFees]   = useState({
    video: tele?.consultationFees?.video ?? 15000,
    audio: tele?.consultationFees?.audio ?? 10000,
    chat:  tele?.consultationFees?.chat  ?? 8000,
  });
  const [types, setTypes] = useState<ConsultType[]>(
    (tele?.consultationTypes as ConsultType[]) ?? ["video", "chat"]
  );
