...
export default function AppointmentCardPast({ appointment, variant }: Props) {
  ...
  <span className="shrink-0 flex items-center gap-1 text-xs font-semibold text-amber-600 bg-amber-50 border border-amber-100 px-2.5 py-1 rounded-full">
    <RefreshCw size={11} />
    En cours d'examen
  </span>
) : (
  <span className="shrink-0 text-xs font-semibold text-slate-500 bg-slate-100 px-2.5 py-1 rounded-full">
    Terminé
          ...
          {/* Message d'attente — RDV payé et manqué, en cours de revue */}
{isUnderReview && (
  <p className="text-xs text-amber-600 bg-amber-50 rounded-lg px-3 py-2">
    Ce rendez-vous n'a pas eu lieu. Notre équipe examine la situation et reviendra vers vous concernant votre paiement.
  </p>
)}
